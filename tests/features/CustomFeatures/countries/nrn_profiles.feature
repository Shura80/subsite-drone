...
Feature: ENRD NRN Profiles
  ...
  @nrn_editor @webmaster @emails @workflow @privacy
  ...
  and if a webmaster rejects the NRN Profile for which I requested publication.
    ...
    And I should see the error message containing "You must agree with the Collection of Data Statement to proceed."
    And I check the box "eu_legal_notice"
    And I check the box "privacy_policy"
    And I press "Ask for publishing"
    ...
    When I am logged in as a user with the "webmaster" role
  ...
  Scenario: As webmaster I manage the NRN editors trough administrative panels.
    Given I am viewing an "nrn_profile" content in "published" status:
